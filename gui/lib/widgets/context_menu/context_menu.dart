...
class _ContextMenuState extends State<ContextMenu>
  ...
  // Whatever had focus right before the menu opened (almost always the
  // anchor button's own internal FocusNode). Restored on close.
  ...
  void _open() {
    ...
    // Deliberately no manual requestFocus() here — the first menu item
    // uses `autofocus: true` instead, which Flutter resolves correctly
    // once the overlay content actually mounts. A postFrameCallback here
    // fires before the overlay's next build, so it would request focus
    // on items that don't exist yet.
  }
