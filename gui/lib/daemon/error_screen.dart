...
final class ErrorScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    if (connectionProvider case AsyncError(:final error)) {
      ...
      if (error is ApplicationError && error.code == AppStatusCode.snapInterfaces) {
...
ErrorData _dataForApplicationError(ApplicationError error) {
  switch (error.code) {
    ...
    case AppStatusCode.snapInterfaces:
      logger.w("Processing a AppStatusCode.snapInterfaces, this should not happen");
      break;
  }
  ...
  if (error.details != null) {
    for (var detail in error.details!) {
      if (detail is Any && detail.typeUrl.endsWith('ErrMissingConnections')) {
        try {
          return ErrMissingConnections.fromBuffer(detail.value).missingConnections;
        } catch (_) {
          return const [];
        }
      }
