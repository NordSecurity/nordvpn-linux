...
func (em *EventManager) OnEvent(event norddrop.Event) {
	...
	if !em.isProd {
		log.Printf(internal.InfoPrefix+" DROP EVENT: %T\n", event.Kind)
	}
	...
	default:
		// TODO: Verify the rest of the events
...
func (em *EventManager) handleRequestReceivedEvent(event norddrop.EventKindRequestReceived) {
	...
	if !peer.DoIAllowFileshare {
		...
		if err := em.fileshare.Cancel(event.TransferId); err != nil {
			log.Printf(internal.ErrorPrefix+" failed to auto-reject transfer %s: %s\n", event.TransferId, err)
		}
		return
	}
	...
	for _, file := range transfer.Files {
		...
		if err != nil {
			log.Println(internal.ErrorPrefix, "failed to autoaccept file:", err)
		}
	}
...
// TODO: Think about handling duplication between this and [handleFileProgressEvent]
func (em *EventManager) handleFileStartedEvent(event norddrop.EventKindFileStarted) {
	...
	if !ok {
		log.Printf(internal.WarningPrefix+" file %s from FileStarted event not found in transfer %s\n",
...
// TODO: Think about handling duplication between this and [handleFileStartedEvent]
func (em *EventManager) handleFileProgressEvent(event norddrop.EventKindFileProgress) {
	...
	file, ok := transfer.Files[event.FileId]
	if !ok {
		log.Printf(internal.WarningPrefix+" file %s from FileProgress event not found in transfer %s\n",
			event.FileId, transfer.ID)
		return
	}
...
func (em *EventManager) handleFileDownloadedEvent(event norddrop.EventKindFileDownloaded) {
	...
	// 	if !ok {
	// 		log.Printf(internal.WarningPrefix+" file %s from TransferFinished event not found in transfer %s\n",
	...
	// 		if err := em.fileshare.Cancel(event.TransferID); err != nil {
	// 			log.Printf(internal.ErrorPrefix+" failed to finalize transfer %s: %s\n", event.TransferID, err)
	// 		}
	// 	}
	// default:
	// 	log.Printf(internal.WarningPrefix+" unknown reason for transfer finished event: %s\n", event.Reason)
	// }
}
...
func (em *EventManager) handleFileUploadedEvent(event norddrop.EventKindFileUploaded) {
	...
	// if err != nil {
	// 	log.Printf(internal.ErrorPrefix+" unmarshalling drop event: %s\n", err)
	// 	return
	// }
	//
	// transfer, err := em.getLiveTransfer(event.TransferID)
...
func (em *EventManager) CancelLiveTransfers() {
	...
	for transferID := range em.liveTransfers {
		...
		if err != nil {
			log.Println(internal.ErrorPrefix, "failed to cancel live transfer:", err)
		}
	}
}
