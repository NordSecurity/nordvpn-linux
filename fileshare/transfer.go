...
func LibdropTransferToInternalTransfer(ti norddrop.TransferInfo) *pb.Transfer {
	...
	out := &pb.Transfer{
		...
		Path:             determineTransferPath2(&ti, allFiles),
	}
...
func determineFullOutgoingPath(outPath *norddrop.OutgoingPath) string {
	...
	case norddrop.OutgoingFileSourceContentUri:
		// TODO: Is this ok?
		return filepath.Join(pathSource.Uri, outPath.RelativePath)
	default:
...
func determineTransferPath2(transferInfo *norddrop.TransferInfo, allFiles []*pb.File) string {
	var result string
			...
			if filepath.Base(path.RelativePath) == path.RelativePath {
				...
				if ok {
					...
					case norddrop.OutgoingFileSourceContentUri:
						...
						transferPath = filepath.Join(source.Uri, dir)
					default:
						log.Printf(internal.WarningPrefix+" unsupported path source: %T\n", source)
					}
				}
...
func determineTransferPath(allFiles []*pb.File) string {
	if len(allFiles) == 0 {
		return ""
	}

	if len(allFiles) == 1 {
		// tranmitting single file, so the transfer path is full file path
		return allFiles[0].FullPath
	}

	splitPaths := make([][]string, len(allFiles))
	for i, file := range allFiles {
		splitPaths[i] = strings.Split(file.FullPath, string(filepath.Separator))
	}

	transferPath := findLongestCommonPrefix(splitPaths)
	if transferPath == "" || transferPath == "/" {
		transferPath = "multiple files"
	}

	return transferPath
}

func findLongestCommonPrefix(splitPaths [][]string) string {
	minLen := len(splitPaths[0])
	for _, parts := range splitPaths {
		if len(parts) < minLen {
			minLen = len(parts)
		}
	}
	...
	return strings.Join(commonComponents, string(filepath.Separator))
}
