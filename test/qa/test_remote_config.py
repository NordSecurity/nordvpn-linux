...
def test_local_files_removal_and_daemon_restart(
    ...
    print(f"Deleting folder: {LOCAL_CACHE_DIR}")
...
def test_local_hash_files_removal_and_daemon_restart(
    ...
    """
    Test to verify the application's behavior when local hash configu files are deleted and the daemon is restarted.
    ...
    Test Steps:
    1. Retrieve all local config files using the "config_manager".
    ...
    rc_hash_files = config_manager.get_local_hash_files()
    ...
    for hash_file in rc_hash_files:
        print(f"Deleting hash file: {hash_file}")
        subprocess.run(["sudo", "rm", hash_file], check=True)
...
def test_hash_modification_and_daemon_restart(
    ...
    for hash_file in rc_hash_files:
        config = config_manager.read_config(hash_file)
        ...
        hash_value_after = config_manager.read_config(hash_file).get("hash")
