...
class TestUtils:
    ...
    def setup_module(ssh_client: ssh.Ssh):
        ...
        TestUtils.allowlist_ssh(ssh_client, "0xe1f1")
    ...
    @staticmethod
    def allowlist_ssh(ssh_client: ssh.Ssh, fwmark: str):
    ...
    @staticmethod
    def ssh_allowlist_rule_args(incoming: bool, fwmark: str) -> list[list[str]]:
        prerouting_port_arg = "--dport" if incoming else "--sport"
        output_port_arg = "--sport" if incoming else "--dport"
        return [
                ["PREROUTING", "-p", "tcp", prerouting_port_arg, "22", "-j", "CONNMARK", "--set-xmark", fwmark],
                ["OUTPUT", "-p", "tcp", output_port_arg, "22", "-j", "MARK", "--set-xmark", fwmark],
        ]
