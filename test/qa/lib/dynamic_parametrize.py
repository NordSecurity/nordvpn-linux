...
def _generate_params_with_ids(argnames, randomized_source, ordered_source, generate_all=False, id_pattern=None, always_pair=None, sample_size=None, ordered_coverage=True):
    """
    ...
    :param ordered_coverage: If True, guarantees that every item in ordered_source is paired exactly once with an
        item from randomized_source
    ...
    if generate_all:
        ...
        if ordered_coverage:
            ords_raw = ordered_source if _is_list_of_lists(ordered_source) else [ordered_source]
            rnds_raw = randomized_source if _is_list_of_lists(randomized_source) else [randomized_source]
            ords_items = list(ords_raw[0]) if len(ords_raw) == 1 and isinstance(ords_raw[0],
                                                                                list | tuple) else ords_raw
            rnds_items = list(rnds_raw[0]) if len(rnds_raw) == 1 and isinstance(rnds_raw[0],
                                                                                list | tuple) else rnds_raw

            if always_pair:
                rnds_items = [item for item in rnds_items if item != always_pair]

            rnd_pool = (rnds_items * (len(ords_items) // len(rnds_items) + 1))[:len(ords_items)]
            random.Random().shuffle(rnd_pool)
            for ord_elem, rnd_elem in zip(ords_items, rnd_pool, strict=False):
                all_combos.append(as_tuple(ord_elem) + as_tuple(rnd_elem))
        else:
            for ord_elem in ords:
...
def dynamic_parametrize(argnames, randomized_source, ordered_source, generate_all=False, id_pattern=None, always_pair=None, sample_size=None, ordered_coverage=True):
    """
    ...
    def decorator(func):
        params, ids = _generate_params_with_ids(argnames, randomized_source, ordered_source, generate_all,
                                                id_pattern, always_pair, sample_size, ordered_coverage)
