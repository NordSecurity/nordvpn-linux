...
def setup_module(module):  # noqa: ARG001
    ...
    peer = meshnet.populate_peer_list_object(sh.nordvpn.mesh.peer.list()).get_internal_peer()
...
def test_accept(accept_directories):
    address = meshnet.populate_peer_list_object(sh.nordvpn.mesh.peer.list()).get_this_device().ip
...
def test_fileshare_transfer(background: bool, peer_name: meshnet.PeerName):
    peer_address = meshnet.get_peer_name(meshnet.populate_peer_list_object(sh.nordvpn.mesh.peer.list()).get_internal_peer(), peer_name)
...
def test_fileshare_transfer_multiple_files_selective_accept(background: bool):
    peer_address = meshnet.populate_peer_list_object(sh.nordvpn.mesh.peer.list()).get_internal_peer().ip
...
def test_fileshare_file_directory_depth_exceeded(background: bool):
    ...
    peer_address = meshnet.populate_peer_list_object(sh.nordvpn.mesh.peer.list()).get_internal_peer()
...
def test_permissions_send_forbidden(peer_name):
    tester_address = meshnet.get_peer_name(meshnet.populate_peer_list_object(sh.nordvpn.mesh.peer.list()).get_this_device(), peer_name)
...
def test_autoaccept():
    peer_list = meshnet.populate_peer_list_object(sh.nordvpn.mesh.peer.list())
...
def test_peers_autocomplete():
    peer_hostname = meshnet.populate_peer_list_object(sh.nordvpn.mesh.peer.list()).get_internal_peer().hostname
