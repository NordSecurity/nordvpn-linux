...
def test_permissions_send(peer_name, background):
    ...
    assert qapeer_permission is not True
...
def test_all_permissions_denied_send_file(background_send: bool, background_accept: bool):
    local_address = meshnet.PeerList.from_str(sh.nordvpn.mesh.peer.list()).get_this_device().hostname
    ...
    wdir = fileshare.create_directory(1)
    peer_address = meshnet.PeerList.from_str(sh.nordvpn.mesh.peer.list()).get_internal_peer().hostname
