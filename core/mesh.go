...
func (api *SimpleClientAPI) Register(token string, appUserID string, peer mesh.Machine) (*mesh.Machine, error) {
	api.mu.Lock()
	defer api.mu.Unlock()
	...
	log.Printf("printing data used for api req: %v", appUserID)
	if err != nil {
		return nil, err
	}
