...
func (fileOpsDefaultImpl) WalkFiles(targetPath, fileExt string, actionFunc func(string)) error {
	err := filepath.WalkDir(targetPath, func(path string, d fs.DirEntry, err error) error {
		if err != nil {
			log.Errorf("accessing %s: %v", path, err)
...
func (f fileOpsDefaultImpl) RenameTmpFiles(targetPath, fileExt string) error {
	return f.WalkFiles(targetPath, fileExt, func(path string) {
		...
		if err := os.Rename(path, newPath); err != nil {
			log.Errorf("renaming %s to %s: %s", path, newPath, err)
		}
	})
}
...
func (f fileOpsDefaultImpl) CleanupTmpFiles(targetPath, fileExt string) error {
	return f.WalkFiles(targetPath, fileExt, func(path string) {
		if err := os.Remove(path); err != nil {
			log.Errorf("removing %s: %s", path, err)
		}
	})
}
...
func (c *CdnRemoteConfig) Load() error {
	...
	if useOnlyLocalConfig {
		log.Info("Ignoring remote config, using only local")
	} else {
...
func (c *CdnRemoteConfig) download() (bool, error) {
	...
	for _, f := range c.features.keys() {
		...
		if err != nil {
			log.Errorf("failed downloading feature [%s] remote config: %v", feature.name, err)
		...
		if dnld {
			...
			log.Infof("feature [%s] remote config downloaded to: %s", feature.name, c.localCachePath)
	...
	for _, f := range c.features.keys() {
		...
		if err := feature.load(c.localCachePath, c.fileOps, jsonValidator{}, c.rcFileOps); err != nil {
			if reportErrors {
				...
				log.Errorf("failed loading feature [%s] config from the disk: %s", feature.name, err)
			}
			continue
		}
		log.Infof("feature [%s] config loaded from: %s", feature.name, c.localCachePath)
