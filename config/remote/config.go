...
func (f *Feature) download(cdn fileReader, fw fileWriter, jv validator, cdnBasePath, targetPath string) (success bool, err error) {
	...
	if f.name == "" {
		return false, &DownloadError{Kind: DownloadErrorOther, Cause: fmt.Errorf("feature name is not set")}
	}

	if err = internal.EnsureDirFull(targetPath); err != nil {
		return false, &DownloadError{Kind: DownloadErrorLocalFS, Cause: fmt.Errorf("setting-up target dir: %w", err)}
	}

	mainJsonHashStr, err := cdn.readFile(f.HashFilePath(cdnBasePath))
	if err != nil {
		return false, &DownloadError{Kind: DownloadErrorRemoteHashNotFound, Cause: fmt.Errorf("downloading main hash file: %w", err)}
	}

	var mainJsonHash jsonHash
	if err = json.Unmarshal(mainJsonHashStr, &mainJsonHash); err != nil {
		return false, &DownloadError{Kind: DownloadErrorHashParsing, Cause: fmt.Errorf("parsing main hash file: %w", err)}
	}
	...
	if err != nil {
		return false, &DownloadError{Kind: DownloadErrorRemoteFileNotFound, Cause: fmt.Errorf("downloading main file: %w", err)}
	}

	// validate json against predefined schema
	if err = jv.validate(mainJsonStr); err != nil {
		return false, &DownloadError{Kind: DownloadErrorParsing, Cause: fmt.Errorf("validating main: %w", err)}
	}
	...
	if !isHashEqual(mainJsonHash.Hash, mainJsonStr, incFiles) {
		return false, &DownloadError{Kind: DownloadErrorHashIntegrity, Cause: fmt.Errorf("main file integrity problem")}
	}
	...
	if err = fw.writeFile(localFileName, mainJsonStr, internal.PermUserRW); err != nil {
		return false, &DownloadError{Kind: DownloadErrorWriteJson, Cause: fmt.Errorf("writing main file: %w", err)}
	}
	...
	if err = fw.writeFile(localFileName, mainJsonHashStr, internal.PermUserRW); err != nil {
		return false, &DownloadError{Kind: DownloadErrorWriteHash, Cause: fmt.Errorf("writing main hash file: %w", err)}
	}
	...
	if err = internal.RenameTmpFiles(targetPath, tmpExt); err != nil {
		return false, &DownloadError{Kind: DownloadErrorFileRename, Cause: fmt.Errorf("writing/renaming files: %w", err)}
	}
