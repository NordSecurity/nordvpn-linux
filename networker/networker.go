...
func (netw *Combined) blockIPv6() {
	...
	// Also block it with iptable rules just to be sure
	err = netw.blockIPv6Traffic()
	if err != nil {
		log.Println(internal.WarningPrefix, "Failed to block ipv6 using iptables", err)
	}
}
...
func (netw *Combined) unblockIPv6() {
	...
	// Also remove the IPv6 blocking rules
	err = netw.unblockIPv6Traffic()
	if err != nil {
		log.Println(internal.WarningPrefix, "Failed to unblock ipv6 using iptables", err)
	}
}

/*
We want to disable IPv6. We already disabled it via sysctl, but
but we left these rules just to be sure.
https://tools.ietf.org/html/rfc4890
Error messages that are essential to the establishment and
maintenance of communications:
-6 -A INPUT              -p ipv6-icmp --icmpv6-type 1   -j DROP
-6 -A INPUT              -p ipv6-icmp --icmpv6-type 2   -j DROP
-6 -A INPUT              -p ipv6-icmp --icmpv6-type 3   -j DROP
-6 -A INPUT              -p ipv6-icmp --icmpv6-type 4   -j DROP
Connectivity checking messages:
-6 -A INPUT              -p ipv6-icmp --icmpv6-type 128   -j DROP
-6 -A INPUT              -p ipv6-icmp --icmpv6-type 129   -j DROP
Address Configuration and Router Selection messages:
-6 -A INPUT              -p ipv6-icmp --icmpv6-type 133 -m hl --hl-eq 255 -j DROP
-6 -A INPUT              -p ipv6-icmp --icmpv6-type 134 -j DROP
-6 -A INPUT              -p ipv6-icmp --icmpv6-type 135 -j DROP
-6 -A INPUT              -p ipv6-icmp --icmpv6-type 136 -j DROP
-6 -A INPUT              -p ipv6-icmp --icmpv6-type 141 -j DROP
-6 -A INPUT              -p ipv6-icmp --icmpv6-type 142 -j DROP
Link-Local Multicast Receiver Notification messages:
-6 -A INPUT -s fe80::/10 -p ipv6-icmp --icmpv6-type 130 -j DROP
-6 -A INPUT -s fe80::/10 -p ipv6-icmp --icmpv6-type 131 -j DROP
-6 -A INPUT -s fe80::/10 -p ipv6-icmp --icmpv6-type 132 -j DROP
-6 -A INPUT -s fe80::/10 -p ipv6-icmp --icmpv6-type 143 -j DROP
SEND Certificate Path Notification messages:
-6 -A INPUT              -p ipv6-icmp --icmpv6-type 148 -j DROP
-6 -A INPUT              -p ipv6-icmp --icmpv6-type 149 -j DROP
Multicast Router Discovery messages:
-6 -A INPUT -s fe80::/10 -p ipv6-icmp --icmpv6-type 151 -j DROP
-6 -A INPUT -s fe80::/10 -p ipv6-icmp --icmpv6-type 152 -j DROP
-6 -A INPUT -s fe80::/10 -p ipv6-icmp --icmpv6-type 153 -j DROP
DHCP6
-6 -A INPUT -d fe80::/64 -p udp -m udp --dport 546 -m comment --comment dhcp6 -j DROP
-6 -A OUTPUT -s fe80::/64 -p udp -m udp --dport 547 -m comment --comment dhcp6 -j DROP
*/
func (netw *Combined) blockIPv6Traffic() error {
	ifaces, err := netw.devices()
	if err != nil {
		return err
	}
	...
	err = netw.fw.Add([]firewall.Rule{
		...
		{
			Name:       "vpn_block_icmp6_multicast",
			Interfaces: ifaces,
...
func (netw *Combined) unblockIPv6Traffic() error {
	err := netw.fw.Delete([]string{
		"vpn_block_icmp6_errors",
		"vpn_block_icmp6_address",
		"vpn_block_icmp6_multicast",
		"vpn_block_dhcp6_in",
		"vpn_block_dhcp6_out",
	})
	if err != nil {
