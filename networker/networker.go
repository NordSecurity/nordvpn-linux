...
func failureRecover(netw *Combined) {
	...
	// TODO: nft - firewall needs to be reconfigured in any case
...
func (netw *Combined) UnsetFirewall() error {
	...
	// refresh the firewall, the netw.fwConfig already contains the correct values
	if err := netw.configureFirewall(netw.fwConfig); err != nil {
...
func (netw *Combined) setMesh(
	...
	defer func() {
		if err != nil {
			...
			// TODO: nft - check what should be in this case
...
func (netw *Combined) UnSetMesh() error {
	...
	if err := netw.configureFirewall(newCfg); err != nil {
		return fmt.Errorf("configure firewall at meshnet stop: %w", err)
	}
...
// 	netw.mu.Lock()
// 	defer netw.mu.Unlock()
// 	if err := netw.allowFileshare(uniqueAddress.UID, uniqueAddress.Address); err != nil {
