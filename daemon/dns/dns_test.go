...
search home`)
	...
	resolvconfResolvconf := []byte(`# Dynamic resolv.conf(5) file for glibc resolver(3) generated by resolvconf(8)
#     DO NOT EDIT THIS FILE BY HAND -- YOUR CHANGES WILL BE OVERWRITTEN

nameserver 110.19.133.218
`)
...
search home`)
	...
	tests := []struct {
		...
		},
		{
			...
			resolvConfLinkTarget:             systemdResolvedLinkTarget,
			setByNmCli:                       true,
			shouldEmitDNSConfiguredEvent:     true,
			expectedManagementServiceInEvent: nmcliManagementService,
		},
		{
			name:                             "resolv.conf comment detection, resolv.conf managed by resolvconf, use resolvconf to set DNS",
			resolvconfFileContents:           resolvconfResolvconf,
			setByResolvconf:                  true,
			shouldEmitDNSConfiguredEvent:     true,
			expectedManagementServiceInEvent: resolvConfManagementService,
		},
		{
		...
		{
			name:                             "resolv.conf symlink detection, resolv.conf is a link to resolvconf, resolvconf is used to set DNS",
			resolvconfFileContents:           noManagerResolvConf,
			resolvConfLinkTarget:             resolvConfLinkTarget,
			setByResolvconf:                  true,
			shouldEmitDNSConfiguredEvent:     true,
			expectedManagementServiceInEvent: resolvConfManagementService,
		},
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			assert.Equal(t, test.setBySystemdResolved, resolvedSetter.isSet,
				"DNS was not configured by the expected setter(resolved).")
			assert.Equal(t, test.setByResolvconf, resolvconfSetter.isSet,
				"DNS was not configured by the expected setter(resolvconf).")
			assert.Equal(t, test.setByNmCli, nmCliSetter.isSet,
				"DNS was not configured by the expected setter(nmcli).")
