...
func TestRecentConnectionsEncode(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			expectedOutput: `{"version":1,"connections":[{"country":"Germany","city":"Berlin","group":11,"country-code":"DE","specific-server-name":"de123","connection-type":6},{"country":"Lithuania","country-code":"LT","connection-type":3}]}`,
		},
	}
...
func TestRecentConnectionsFileMigration(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "non obfuscated groups are not affected",
			connections: []Model{
				{
					Country:            "Germany",
					CountryCode:        "DE",
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			connections := migrateToVersion1(test.connections)
			assert.Equal(t, test.expectedOutput, connections)
		})
	}
}
