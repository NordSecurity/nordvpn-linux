...
func (ExecCommandExecutor) ExecuteCommand(command string) error {
	commandArgs := strings.Split(command, "")
	_, err := exec.Command(commandArgs[0], commandArgs[1:]...).CombinedOutput()
	return err
}
...
func (f *FirewallManager) FileshareAllow(peer meshnet.UniqueAddress) error {
	...
	if err := f.commandExecutor.ExecuteCommand("iptables -I " + rule); err != nil {
...
func (f *FirewallManager) FileshareDeny(peer meshnet.UniqueAddress) error {
	...
	if err := f.commandExecutor.ExecuteCommand("iptables -D " + rule); err != nil {
...
func (f *FirewallManager) BlockTraffic() error {
	...
	for _, iface := range interfaces {
		inputCommand := fmt.Sprintf("iptables -I INPUT -i %s -m comment --comment nordvpn -j DROP", iface.Name)
		if err := f.commandExecutor.ExecuteCommand(inputCommand); err != nil {
			return fmt.Errorf("blocking input traffic: %w", err)
		}

		outputCommand := fmt.Sprintf("iptables -I OUTPUT -o %s -m comment --comment nordvpn -j DROP", iface.Name)
...
func (f *FirewallManager) UnblockTraffic() error {
	...
	for _, iface := range interfaces {
		inputCommand := fmt.Sprintf("iptables -D INPUT -i %s -m comment --comment nordvpn -j DROP", iface.Name)
		if err := f.commandExecutor.ExecuteCommand(inputCommand); err != nil {
			return fmt.Errorf("blocking input traffic: %w", err)
		}

		outputCommand := fmt.Sprintf("iptables -D OUTPUT -o %s -m comment --comment nordvpn -j DROP", iface.Name)
		if err := f.commandExecutor.ExecuteCommand(outputCommand); err != nil {
			return fmt.Errorf("blocking output traffic: %w", err)
		}
	}
...
func (f *FirewallManager) AllowIncoming(peer meshnet.UniqueAddress, allowLocal bool) error {
	...
	if !allowLocal {
		...
		for _, lan := range lans {
			...
			if err := f.commandExecutor.ExecuteCommand("iptables -I " + blockLANRule); err != nil {
...
func (f *FirewallManager) DenyIncoming(peer meshnet.UniqueAddress) error {
	...
	if err := f.commandExecutor.ExecuteCommand("iptables -D " + rule.allowIncomingRule); err != nil {
		return fmt.Errorf("adding allow incoming rule: %w", err)
	}

	for _, blockLANCommand := range rule.blockLANRules {
		if err := f.commandExecutor.ExecuteCommand("iptables -D " + blockLANCommand); err != nil {
...
func (f *FirewallManager) UnblockMeshnet(deviceAddress string) error {
	...
	command := fmt.Sprintf("iptables -I INPUT -s 100.64.0.0/10 -m conntrack --ctstate RELATED,ESTABLISHED --ctorigsrc %s -m comment --comment nordvpn -j ACCEPT", deviceAddress)
	if err := f.commandExecutor.ExecuteCommand(command); err != nil {
		return fmt.Errorf("blocking unrelated mesh traffic: %w", err)
	}

	err := f.commandExecutor.ExecuteCommand("iptables -I INPUT -s 100.64.0.0/10 -m comment --comment nordvpn -j DROP")
	if err != nil {
...
func (f *FirewallManager) allowlistPort(iface string, protocol string, portRange PortRange) error {
	...
	if err := f.commandExecutor.ExecuteCommand("iptables -I " + inputDportRule); err != nil {
	...
	if err := f.commandExecutor.ExecuteCommand("iptables -I " + inputSportRule); err != nil {
	...
	if err := f.commandExecutor.ExecuteCommand("iptables -I " + outputDportRule); err != nil {
	...
	if err := f.commandExecutor.ExecuteCommand("iptables -I " + outputSportRule); err != nil {
		return fmt.Errorf("allowlisting input dport: %w", err)
	}
...
func (f *FirewallManager) SetAllowlist(udpPorts []int, tcpPorts []int, subnets []string) error {
	...
	for _, subnet := range subnets {
		for _, iface := range ifaces {
			...
			if err := f.commandExecutor.ExecuteCommand("iptables -I " + inputRule); err != nil {
			...
			if err := f.commandExecutor.ExecuteCommand("iptables -I " + outputRule); err != nil {
...
func (f *FirewallManager) manageApiAllowlist(allow bool) error {
	iptablesMode := "iptables -I "
	if !allow {
		iptablesMode = "iptables -D "
	}
