...
func testRPC() *RPC {
	...
	dm.SetServersData(time.Now(), core_test.ServersList(), "")
	...
	return NewRPC(
		...
		dm,
		api,
		core_test.NewMockServersAPI(),
		&core_test.CredentialsAPIMock{},
		&core_test.DedicatedServersAPIMock{},
		testNewCDNAPI(),
		testNewRepoAPI(),
		&core_test.AuthenticationAPImock{},
		"1.0.0",
...
func waitPortForListener(port int, timeoutSec int) (net.Listener, error) {
	passedTime := 0
	log.Infof("Waiting for port: %d to become available...", port)
	for {
		passedTime++
		listener, err := net.Listen("tcp", ":"+strconv.Itoa(port))
		if listener != nil {
			log.Infof("Port: %d is available!", port)
			return listener, nil
		}
