...
func JobServers(dm *DataManager, api core.ServersAPI, validate bool) func() error {
	return func() error {
		...
		dist := distance(
			...
			servers[0].Locations[0].Country.City.Latitude,  //nolint:staticcheck
			servers[0].Locations[0].Country.City.Longitude, //nolint:staticcheck
		)
		...
		for idx, server := range servers {
			...
			dist := distance(
				geoInfoData.Insights.Latitude,
				geoInfoData.Insights.Longitude,
				country.City.Latitude,  //nolint:staticcheck
				country.City.Longitude, //nolint:staticcheck
			)
