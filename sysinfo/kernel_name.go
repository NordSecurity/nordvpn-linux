...
func uname(runner cmdRunner, flags string) string {
	...
	if err != nil {
		log.Errorf("failed to execute 'uname %s': %v. Falling back to default: %s", flags, err, defaultKernelName)
		return defaultKernelName
	}

	if out == "" {
		log.Errorf("'uname %s' returned empty output. Falling back to default: %s", flags, defaultKernelName)
		return defaultKernelName
	}
