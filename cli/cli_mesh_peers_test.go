		...
		Response: &pb.GetPeersResponse_Peers{
			Peers: &pb.PeerList{
				...
				Local: []*pb.Peer{{
					...
					Hostname:         "IAllowInbound_IAllowRouting",
...
func TestMeshPeerList(t *testing.T) {
	...
	c := cmd{nil, mockMeshClient{}, nil, "", nil, config.Config{}, nil, false}
	tests := []struct {
		name     string
		expected []string
	}{
		...
		{
			name:     "offline",
			expected: []string{"IAllowInbound_IAllowRouting", "IsRoutable"},
		},
		{
			name:     "internal",
			expected: []string{"IAllowInbound_InboundAllowed_IsRoutable", "IAllowInbound_IAllowRouting"},
		},
		{
			name:     "external",
		...
		},
		{
			...
			expected: []string{"IAllowInbound_InboundAllowed_IsRoutable", "IAllowInbound_IAllowRouting", "AllowsEverything"},
		},
		{
			name:     "routing-allowed",
			expected: []string{"AllowsEverything", "IAllowInbound_IAllowRouting"},
		},
		{
